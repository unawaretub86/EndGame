...
export default function AdminProject({ dataID }) {
    ...
    return (
        <>{ stAdminProject.status ?
            ...
            <Typography> Project Leader: {stAdminProject.leader.name.concat(' ',stAdminProject.leader.lastName)}</Typography>
            <Typography> Project Status: {stAdminProject.status}</Typography>
            <Typography> Project Phase: {stAdminProject.phase}</Typography>
            <pre>{JSON.stringify(data, null, 2)}</pre>
            <Button variant="outlined" onClick={hdlActivation}>{ stAdminProject.status === 'inactive' ? 'Activate':'Inactivate'}</Button>         
            </>
            : <p>Loading......</p>
        }
        </>
